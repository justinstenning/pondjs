...
it("can create a new range with a begin and end time", () => {
    ...
    const range = timerange(beginTime, endTime);
    expect(range.begin().getTime()).toBe(beginTime.getTime());
    expect(range.end().getTime()).toBe(endTime.getTime());
});

it("can create a new range with two millisecond timestamps", () => {
...
it("can be used to give a new range", () => {
    ...
    const rangeOrig = timerange(beginTime, endTime);
    const rangeCopy = timerange(rangeOrig);
...
it("can output JSON in the correct format", () => {
    ...
    const range = timerange(beginTime, endTime);
    expect(range.toJSON()).toEqual({ timerange: [1326280260000, 1329912720000] });
});
...
it("can output a string representation", () => {
    ...
    const endTime = moment.utc("2012-02-22 12:12", fmt).toDate();
    const range = timerange(beginTime, endTime);
    expect(range.toString()).toBe('{"timerange":[1326280260000,1329912720000]}');
});
...
it("can be undefined if the ranges don't intersect", () => {
    ...
    const rangeOutside = timerange(beginTimeOutside, endTimeOutside);
...
it("can be a new range if the ranges intersect", () => {
    ...
    const rangeOverlap = timerange(beginTimeOverlap, endTimeOverlap);
    const expected = timerange(beginTimeOverlap, endTime);
...
it("can be a new range (the smaller range) if one range surrounds another", () => {
    ...
    const rangeInside = timerange(beginTimeInside, endTimeInside);
